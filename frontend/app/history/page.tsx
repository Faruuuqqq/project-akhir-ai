    ...
    <div className="bg-clinical-pearl font-inter text-charcoal">
      <main className="mx-auto max-w-6xl px-6 pt-32 pb-32">
        <nav className="flex items-center gap-2 mb-8 text-xs font-mono text-slate" aria-label="Breadcrumb">
          ...
          <span className="text-trust-teal font-bold">Riwayat Analisis</span>
        </nav>
        ...
        {history.length === 0 ? (
          ...
          >
            ...
            <Link
              href="/screening"
              className="inline-flex items-center gap-2 bg-trust-teal text-white px-6 py-3 rounded-lg font-jakarta font-semibold hover:bg-teal-dark transition-all"
            >
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-light-silver bg-clinical-pearl">
                      <th className="px-6 py-4 text-left font-jakarta text-xs font-bold uppercase tracking-[0.1em] text-charcoal">Tanggal</th>
                      <th className="px-6 py-4 text-left font-jakarta text-xs font-bold uppercase tracking-[0.1em] text-charcoal">ID Pasien</th>
                      <th className="px-6 py-4 text-left font-jakarta text-xs font-bold uppercase tracking-[0.1em] text-charcoal">BI-RADS</th>
                      <th className="px-6 py-4 text-left font-jakarta text-xs font-bold uppercase tracking-[0.1em] text-charcoal">Probabilitas</th>
                      <th className="px-6 py-4 text-left font-jakarta text-xs font-bold uppercase tracking-[0.1em] text-charcoal">Temuan</th>
                      <th className="px-6 py-4 text-center font-jakarta text-xs font-bold uppercase tracking-[0.1em] text-charcoal">Aksi</th>
                    </tr>
                  </thead>
                  <tbody>
                    {history.map((record) => (
                      <tr key={record.id} className="border-b border-light-silver hover:bg-clinical-pearl/30 transition-colors">
                        <td className="px-6 py-4 text-sm text-charcoal font-medium">{record.date}</td>
                        <td className="px-6 py-4 text-sm font-mono text-slate">{record.patientId || '-'}</td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span className="text-sm font-mono font-bold text-charcoal">{record.probabilityPercent}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm text-slate">{record.findingsCount} temuan</td>
                        <td className="px-6 py-4 text-center">
                          <button
                            onClick={() => setDeleteConfirmId(record.id)}
                            className="inline-flex items-center gap-1 text-xs font-semibold text-muted-rose hover:text-muted-rose/80 transition-colors"
                          >
